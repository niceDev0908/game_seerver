import { IUserInfo, UserInfo } from './types/IUser';
...
io.on(IdSocketKey.connection, (client: any) => {
    ...
    client.on('create', (data: UserInfo) => {
        console.log('create')
        if(room[data.roomId]) {
            client.emit('exist');
            return;
        ...
        room[data.roomId] = [];
        room[data.roomId].push(client);
        client.join(data.roomId);
        client.emit('created');
    });

    client.on('join', (data: UserInfo) => {
        if(!room[data.roomId]) {
            client.emit('not_exist');
            return;
        }
        if(room[data.roomId].length>1) {
            client.emit('full');
            return;
        }
        room[data.roomId][0].emit('joined');
        room[data.roomId] = [];
        room[data.roomId].push(client);
        client.join(data.roomId);
        client.emit('joined');
